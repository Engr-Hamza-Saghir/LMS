    ...
    public void dataque() {
        ...
        JsonArrayRequest jsonArray = new JsonArrayRequest(Request.Method.GET, url, null, new Response.Listener<JSONArray>() {
            ...
            public void onResponse(JSONArray response) {
                Log.d("chunk", "For loop started" + token);
                ...
                try {
                    ...
                    for (int j = 0; j <= jsonArray1.length(); j++) {
                        ...
                        if (jsonObject.getString("name").contains("Assignment")||jsonObject.getString("name").contains("Assigment")) {
                            assgn_name.setText(jsonObject.getString("name"));
                            assgn_url.setText(jsonObject.getString("url"));
                            JSONArray assignment_dates = jsonObject.getJSONArray("dates");
                            for (int k = 1; k < assignment_dates.length(); k++) {
                                JSONObject due_date = assignment_dates.getJSONObject(k);
                                assignment_due_date = due_date.getString("timestamp");
                                Log.d("dateee", "dataque: " + token);
                            ...
                            if (token.equals("8e6dc0f1606847131b60cc511d36db23")) {
                                cardView.setOnClickListener(new View.OnClickListener() {
                                    ...
                                    public void onClick(View view) {
                                        ...
                                        intent.putExtra("assignment_due_date", assignment_due_date);
